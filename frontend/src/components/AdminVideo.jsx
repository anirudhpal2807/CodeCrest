...
const AdminVideo = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const message =
        err.response?.data?.error || err.response?.data?.message || err.message || 'Failed to delete video';
      setError(message);
      console.error(err);
    }
  };
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-[var(--cc-bg-secondary)]">
                  ...
                  <th className="text-right text-xs font-semibold text-[var(--cc-text-muted)] uppercase tracking-wider px-6 py-4 w-56">
                    Actions
                  </th>
                </tr>
              </thead>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2">
                        <NavLink
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium text-[var(--cc-primary-light)] bg-[rgba(99,102,241,0.1)] hover:bg-[rgba(99,102,241,0.2)] border border-[rgba(99,102,241,0.2)] transition-colors"
                        >
                          <Upload size={14} />
                          Upload
                        </NavLink>
